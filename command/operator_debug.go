...
func (c *OperatorDebugCommand) collectAgentHost(path, id string, client *api.Client) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), api.PermissionDeniedErrorContent) {
			...
			c.Ui.Warn("Agent host retrieval requires agent:read ACL or enable_debug=true.  See https://openwander.org/wander/api-docs/agent#host for more information.")
		}
...
func (c *OperatorDebugCommand) collectPprof(path, id string, client *api.Client, interval int) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), api.PermissionDeniedErrorContent) {
			...
			c.Ui.Warn("Pprof retrieval requires agent:write ACL or enable_debug=true.  See https://openwander.org/wander/api-docs/agent#agent-runtime-profiles for more information.")
