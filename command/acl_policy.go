...
func (f *ACLPolicyCommand) Help() string {
	helpText := `
Usage: wander acl policy <subcommand> [options] [args]

  This command groups subcommands for interacting with ACL policies. Wander's ACL
  ...
  see: https://openwander.org/wander/guides/acl.html

  Create an ACL policy:

      $ wander acl policy apply <name> <policy-file>

  List ACL policies:

      $ wander acl policy list

  Inspect an ACL policy:

      $ wander acl policy info <policy>
